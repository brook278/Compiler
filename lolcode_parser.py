...
def build_parser(possible_tokens):
    ...
    @pg.production(
        'statement : IM IN YR LOOP optional_assignment_expression optional_til_expression newlines code_block NOW IM OUTTA YR LOOP')
    def loop(p):
    ...
    @pg.production('expression : I IZ var_use optional_arg_call MKAY')
    def func_call(p):
        return FunctionCall(p[2], p[3])

    @pg.production('statement : HOW IZ I var_use optional_argument MKAY newlines code_block function_end')
    def func_statement(p):
        name = p[3]
        lines = p[7]
        arguments = p[4]
        func_type = p[8]
        return FunctionDefinition(name, lines, arguments, func_type)

    @pg.production('optional_arg_call : YR expression optional_an optional_arg_call')
    def opt_func_call_arg(p):
        if p[3] is not None:
            return [p[1]] + p[3]
        else:
            return [p[1]]

    @pg.production('optional_arg_call : ')
    def empty_opt_func_call_arg(p):
        return

    @pg.production('optional_argument : YR var_use literal_type_declaration optional_an optional_argument')
    @pg.production('optional_argument : YR var_use array_type_declaration optional_an optional_argument')
    def opt_func_arg(p):
        if p[4] is not None:
            return [[p[1]] + [p[2]]] + p[4]
        else:
    ...
    @pg.production('optional_argument : ')
    def opt_func_arg_empty(p):
        return None

    @pg.production('function_end : IF U SAY SO literal_type_declaration')
    @pg.production('function_end : IF U SAY SO array_type_declaration')
    def func_return(p):
        return p[4]

    @pg.production('statement : FOUND YR expression')
    def func_return(p):
        return FunctionReturn(p[2])
