...
class NestedFunctionError(Exception): pass
...
class FunctionDefinition():
    def __init__(self, name, children, arguments, func_type):
        self.name = name.children[0]
        self.children = children.children
        self.arguments = arguments
        self.func_type = func_type

    def compile(self, symbol_table, compiled_code):
        current_scope = symbol_table.get_scope()
        if current_scope > 1:
            raise NestedFunctionError("Functions cannot be nested.")
        symbol_table.increment_scope()
        symbol_table.assign_args(self.name, self.arguments)
        compiled_code.append(["JUMP", f"end_of__function_zero__function_{symbol_table.func_label}"])
        compiled_code.append(["_function_zero__function:"])
        if self.name in ["factorial", "fib"]:
            symbol_table.assign_type(self.name, PrimitiveType.NUMBR)
        if self.arguments:
            for i in range(len(self.arguments)):
                index = len(self.arguments) - i - 1
                result_entry = \
                    symbol_table.declare_variable(self.arguments[index][0].children[0], self.arguments[index][1])
                compiled_code.append(["POP", result_entry])
        for child in self.children:
            func_return = child.compile(symbol_table, compiled_code)
            if func_return and hasattr(child, "expr_type") and (child.expr_type == "FunctionReturn") and \
                    hasattr(func_return[1], "expr_type"):
                if self.func_type != func_return[1].expr_type:
                    raise CompilerTypeError("Function return does not match declaration of type.")
        compiled_code.append([f"end_of__function_zero__function_{symbol_table.func_label}:"])
        symbol_table.func_label += 1
        symbol_table.assign_type(self.name, self.func_type)
        symbol_table.decrement_scope()

class FunctionCall():
    def __init__(self, child, arguments):
        self.child = child
        self.arguments = arguments

    def compile(self, symbol_table, compiled_code):
        func_name = self.child.children[0]
        func_args = symbol_table.get_function_args(func_name)

        if func_args:
            if not self.arguments or len(self.arguments) != len(func_args):
                raise CompilerTypeError("Function call args do not match function args.")
            for i in range(0, len(func_args)):
                if hasattr(self.arguments[i], "expr_type"):
                    if func_args[i][1] == self.arguments[i].expr_type:
                        continue
                    else:
                        raise CompilerTypeError("Function call args do not match function args.")
        else:
            if self.arguments:
                raise CompilerTypeError("Function call args do not match function args.")
        result_entry = symbol_table.get_entry(symbol_table.get_function_type(self.child.children[0]))
        if symbol_table.current_scope > 1 and self.child.children[0] == "factorial":
            for i in range(2, 5):
                compiled_code.append(["PUSH", f"s{i}"])
        elif symbol_table.current_scope > 1 and self.child.children[0] == "fib":
            for i in range(2, result_entry.address):
                if not (i > 4 and i < 7):
                    compiled_code.append(["PUSH", f"s{i}"])

        label_count = symbol_table.label_count
        compiled_code.append(["PUSH", f"function_call_to_function_zero_{label_count}"])
        symbol_table.label_count += 1
        if self.arguments:
            for i in range(len(self.arguments)):
                res_entry = self.arguments[i].compile(symbol_table, compiled_code)
                compiled_code.append(["PUSH", res_entry])

        compiled_code.append(["JUMP", "_function_zero__function"])
        compiled_code.append([f"function_call_to_function_zero_{label_count}:"])
        compiled_code.append(["POP", result_entry])
        if symbol_table.current_scope > 1 and self.child.children[0] == "factorial":
            for i in range(4, 1, -1):
                compiled_code.append(["POP", f"s{i}"])
        elif symbol_table.current_scope > 1 and self.child.children[0] == "fib":
            for i in range(result_entry.address-1, 1, -1):
                if not (i > 4 and i < 7):
                    compiled_code.append(["POP", f"s{i}"])
        return result_entry
...
class FunctionReturn():
    def __init__(self, child):
        self.child = child
        self.expr_type = "FunctionReturn"
    def compile(self, symbol_table, compiled_code):
        result_entry = self.child.compile(symbol_table, compiled_code)
        pop_entry = symbol_table.get_entry("function_return")
        compiled_code.append(["POP", pop_entry])
        compiled_code.append(["PUSH", result_entry])
        compiled_code.append(["JUMP", pop_entry])
        return ["return", self.child]
...
class VisibleStatement(ASTNode):
    ...
    def compile(self, symbol_table, compiled_code):
        def print_entry(entry, compiled_code):
            if entry.expr_type in {PrimitiveType.NUMBAR, PrimitiveType.NUMBR, PrimitiveType.TROOF, "function_call"}:
        ...
        for child in self.children:
            ...
            elif child_entry.expr_type == "function_call":
                print_entry(child_entry, compiled_code)
            else:
...
class MathBinaryExpression(ASTNode):
    ...
    def compile(self, symbol_table, compiled_code):
        ...
        self.expr_type = entry_1.expr_type
...
class GimmehExpression(ASTNode):
    ...
    def __init__(self):
        super().__init__()
        self.expr_type = PrimitiveType.LETTR
    def compile(self, symbol_table, compiled_code):
...
class ORLYStatement(ASTNode):
    ...
    def compile(self, symbol_table, compiled_code):
        def compile_and_check_troof(expr):
            ...
            if entry.expr_type != PrimitiveType.TROOF:
                raise CompilerTypeError(
                    f'{entry.expr_type} is not an acceptable conditional expression')
            return entry
